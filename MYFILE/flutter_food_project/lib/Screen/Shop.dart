...
import 'package:flutter_food_project/Screen/cart.dart';
...
class _ShopScreenState extends State<ShopScreen> {
  ...
  void updateCart(List<Map<String, dynamic>> updatedCart) {
    setState(() {
      cart = updatedCart;  // อัปเดตตะกร้าใหม่จากข้อมูลที่ได้รับ
    });
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () async {
              // เปิดหน้าตะกร้าสินค้า และส่งข้อมูลตะกร้าไป
              final updatedCart = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => CartScreen(cart: cart), // ส่งตะกร้าไปด้วย
                ),
              );

              // หากได้รับข้อมูลตะกร้าที่อัปเดตจาก CartScreen
              if (updatedCart != null) {
                updateCart(updatedCart);  // อัปเดตตะกร้าใน ShopScreen
              }
            },
            icon: Stack(
              children: [
                              ...
                              onPressed: () {
                                setState(() {
                                  cart.add({
                                      ...
                                      'price': pizzas[i]["price"],
                                      'discountPrice': pizzas[i]["discountPrice"],
                                      'email': widget.email,  // เพิ่ม email ไปพร้อมกับสินค้าตัวนี้
                                  });
                                });
                              },
